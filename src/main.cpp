...
// Código para leer la entrada adc desde el pin IO13 y mostrar los valores crudos y escalados en las barras de la pántalla 02
...
void loop()
{
  ...
  if (g_eez_event_is_available == true)
  {
    ...
    // Código para leer el texto de la pantalla y enviarlo al monitor serie
    else if (obj == objects.screen03_textarea_kb)
    {
      setBuzzerFlag = true;
      Serial.printf("Texto en TextArea: %s\n", lv_textarea_get_text(objects.screen03_textarea_kb));
      
    }
  }

  //! ===== Inicio código para manejar frecuencia de sonido del buzzer. Buzzer feedback for button press ======
  static uint32_t buzzerTimerPrev = 0L;
  static boolean buzzerActive = false;

  if(setBuzzerFlag == true && !buzzerActive)
  {
    buzzerTimerPrev = millis(); 
    setBuzzerFlag = false;
    buzzerActive = true;
    digitalWrite(BUZZER_OUTPUT_PIN, HIGH);
  }

  if(buzzerActive && millis() - buzzerTimerPrev > 75)
  {
    buzzerActive = false;
    digitalWrite(BUZZER_OUTPUT_PIN, LOW);
  }
  ...
  // ! ============ Inicio código para leer la entrada adc desde el pin IO13 ===================

  if (lv_screen_active() == objects.screen02) // Este código se ejecutará sólo cuando la pantalla actual sea la pantalla 02
  {
    static int intPotValPrev = analogRead(ANALOG_INPUT_PIN); // Obtenemos el valor inicial del potenciómetro

    int potValCur = analogRead(ANALOG_INPUT_PIN);       // Leemos el valor actual crudo del potenciómetro
    if (abs(potValCur - intPotValPrev) > 5) // Actualizamos sólo si el cambio es mayor a 5 unidades para evitar fluctuaciones menores
    {
  ...
  // ! ============ Fin código para leer la entrada adc desde el pin IO13 ===================
